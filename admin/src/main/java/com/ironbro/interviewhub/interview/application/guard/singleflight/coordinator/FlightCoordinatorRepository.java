package com.ironbro.interviewhub.interview.application.guard.singleflight.coordinator;

import cn.hutool.core.util.StrUtil;
import com.ironbro.interviewhub.interview.application.guard.singleflight.model.FlightAcquireResult;
import com.ironbro.interviewhub.interview.application.guard.singleflight.model.FlightAction;
...
public class FlightCoordinatorRepository {
     ...
     *   2. status=SUCCEEDED → REPLAY_SUCCESS（已有人跑完，直接拿结果）
     *   3. status=FAILED 且 retryable=1 → OWNER_TAKEOVER（重试接管）
     *   4. status=FAILED 且 retryable=0 → REPLAY_FAILURE（不可重试，抛异常）
     *   5. status=PENDING/RUNNING，检查心跳：
     *      - heartbeatAt 在 takeoverDetectMillis 内 → FOLLOWER_WAIT（owner 还活着）
     *      - heartbeatAt 超时 → OWNER_TAKEOVER（owner 挂了，我来接管）
     *
     * KEYS[1]=meta key（状态在哪），KEYS[2]=owner-seq（全局计数器，生成唯一 ownerToken）
     */
    public FlightAcquireResult acquireOrJoin(String stage, String requestKey, String ownerId, String sessionId,
        ...
        String raw = stringRedisTemplate.execute(
                ...
                List.of(metaKey(requestKey), OWNER_SEQ_KEY),  // KEYS: meta key + 全局计数器
                StrUtil.blankToDefault(stage, "interview-default"),       // ARGV[1]
                StrUtil.blankToDefault(ownerId, "unknown-node"),          // ARGV[2] 节点身份
                StrUtil.blankToDefault(requestKey, "no-key"),             // ARGV[3]
                StrUtil.blankToDefault(sessionId, "no-session"),          // ARGV[4]
                String.valueOf(now),                                      // ARGV[5] 当前时间
                String.valueOf(now + runningTtlMillis),                   // ARGV[6] 过期时间
                String.valueOf(runningTtlMillis),                         // ARGV[7] 运行 TTL
                String.valueOf(policy == null ? 10000L : policy.getTakeoverDetectMillis()) // ARGV[8] 心跳超时阈值
        );
        return parseAcquireResult(raw); // 解析 Lua 返回值，如 "OWNER_NEW|42"
    }
